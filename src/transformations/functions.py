...
def print_metrics(test_target, predictions):
    accuracy, precision, recall, f1, roc_auc, conf_matrix = eval_classification_model(
...
def profit_calculation(df, stake=1):
    ...
    print(
        f"Total return from betting £{stake:.2f} on each prediction where model_preds == 1: £{total_return:.2f}"
    )
...
def monte_carlo_sim(df_race, n_sims=10000, seed=42):
    ...
    for _ in range(n_sims):
        winner = rng.choice(df_race.index, p=df_race["pred_prob"] / df_race["pred_prob"].sum())
        win_counts[winner] += 1
